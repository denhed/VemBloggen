...
import se.vem.databas.CommentsMapper;
...
public class Test {
	...
	private static void addComments() {
		CommentsMapper commentsMapper = CommentsMapper.getInstance();

		Comments c = new Comments();
		c.setPoster("Robin");
		c.setText("Kommentaaaaaaaaaaaaar");
		commentsMapper.addComment(c);
	}

	private static void listComments() {
		CommentsMapper commentsMapper = CommentsMapper.getInstance();
		List<Comments> comments = commentsMapper.listComments(); // vi får alla
																	// obj i db.
		int i = 1;
		for (Comments listComments : comments) {
			System.out.print(i++ + " ");
	...
	private static void listAllBlogs() {
		...
		for (Blogs listBlogs : blogs) {
			System.out.print(listBlogs.getBlog_id() + " "); // skriver ut
															// blog_id.
			System.out.println(listBlogs.getTitle()); // Skriver ut titeln på
														// bloggen.
		}
	}
